<header class="hidden lg:mx-9 mx-4 py-4 lg:flex items-center justify-between">
    <nav class="">
      <ul class="flex items-center justify-center">
        <li class="relative group py-2"><button
            class="curseur active focus:text-black focus:font-bold extra antialiased text-xs pr-2 py-2 rounded-md hover:font-bold"
            aria-haspopup="true"> SHOP </button>
          <div
            class="w-screen absolute -left-9 top-3 transition group-hover:translate-y-[13px] translate-y-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-700 ease-in-out group-hover:transform z-50 min-w-[560px] transform">
            <div class="invisible pt-8">fafefefefaefaefaefaeffefaefef</div>
            <div class="relative top-6 pl-9 p-6 pb-7 bg-white border-b border-t border-tiret2 w-screen">
              <div class="relative z-10">
                <div class="flex justify-start gap-56">


                    <div>
                    <ul class="">
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 1, 'slug' => slugCategory(1)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 font-semibold hover:text-indigo-600">
                          NEW DROP </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 2, 'slug' => slugCategory(2)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 font-semibold hover:text-indigo-600">
                          KEEP*IT*FLY </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 3, 'slug' => slugCategory(3)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 font-semibold hover:text-indigo-600">
                          SHOP INSTAGRAM </a></li>
                      {{-- <li><a href="{{ route('get_products_by_categories', ['category_id' => 4]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 font-semibold hover:text-indigo-600">
                          TEE-SHIRTS </a></li> --}}
                    </ul>
                  </div>


                  <div class="grid grid-cols-3 gap-[88px]">
                    <ul class="">
                      <li><a href="#"
                          class="hover:opacity-60 font-bold text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                          CLOTHING </a></li>
                          <li class="pt-2"><a href="{{route('shop')}}"
                            class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                            ALL </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 4, 'slug' => slugCategory(4)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                          T-SHIRTS </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 3, 'slug' => slugCategory(3)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                          HOODIES </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 1, 'slug' => slugCategory(1)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                          PANTS </a></li>

                    </ul>
                    <ul class="">
                      <li><a href="#"
                          class="hover:opacity-60 font-bold text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                          ACCESSORIES </a></li>

                          <li>
                            <a href="{{ route('get_products_by_categories', ['category_id' => 2, 'slug' => slugCategory(2)]) }}"
                                class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                                ALL </a>
                          </li>
                          <li>
                            <a href="#"
                                class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                                CAPS </a>
                          </li>
                    </ul>
                    <ul class="">
                      <li><a href="{{route('overview')}}"
                          class="hover:opacity-60 font-bold text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                          COLLECTIONS </a></li>

                          <a href="{{route('shop')}}"
                        class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800 hover:text-indigo-600">
                        KEEP*IT*FLY </a>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </li>
        <li class="relative group py-2"><button
            class="curseur focus:text-black focus:font-bold extra antialiased text-xs px-2 py-2 rounded-md hover:font-bold"
            aria-haspopup="true"> COLLECTIONS </button>
          <div
            class="w-screen absolute -left-[80px] top-3 transition group-hover:translate-y-[13px] translate-y-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-700 ease-in-out group-hover:transform z-50 min-w-[560px] transform">
            <div class="invisible pt-8">fafefefefaefaefaefaeffefaefef</div>
            <div class="relative top-6 pl-9 p-6 pb-7 bg-white border-b border-t border-tiret2 w-screen">
              <div class="relative z-10">
                <div class="flex justify-start gap-56">
                  <div>
                    <ul class="">
                      <li><a href="{{route('shop')}}"
                          class="hover:opacity-60 font-bold text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                          KEEP*IT*FLY </a></li>
                      <li><a href="{{ route('overview') }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                          OVERVIEW </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 4, 'slug' => slugCategory(4)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                          T-SHIRTS </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 3, 'slug' => slugCategory(3)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                          HOODIES </a></li>
                        <li><a href="{{ route('get_products_by_categories', ['category_id' => 1, 'slug' => slugCategory(1)]) }}"
                        class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                        PANTS </a></li>
                      <li><a href="{{ route('get_products_by_categories', ['category_id' => 2, 'slug' => slugCategory(2)]) }}"
                          class="hover:opacity-60 text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                          ACCESSORIES </a></li>
                    </ul>
                    <div class="mt-4 flex justify-center items-center"><img class="w-[314px] h-[337px]"
                        src="{{asset('files/slider_home01.6ec5af8b.jpg')}}" alt=""></div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </li>
        <li class="relative group py-2"><button
            class="curseur focus:text-black focus:font-bold extra antialiased text-xs px-2 py-2 rounded-md hover:font-bold"
            aria-haspopup="true"> MEDIA </button>
          <div
            class="w-screen absolute -left-44 top-3 transition group-hover:translate-y-[13px] translate-y-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-700 ease-in-out group-hover:transform z-50 min-w-[560px] transform">
            <div class="invisible pt-8">fafefefefaefaefaefaeffefaefef</div>
            <div class="relative top-6 pl-9 p-4 pb-7 bg-white border-b border-t border-tiret2 w-screen">
              <div class="relative z-10">
                <div class="flex justify-start gap-56">
                  <div>
                    <ul class="">
                      <div class="flex justify-center items-center gap-x-3">
                        <div class="hover:opacity-70"><img class="w-[314px] h-[337px]"
                            src="{{asset('files/slider_home01.6ec5af8b.jpg')}}" alt="">
                          <div class="mt-[18px]">
                            <p class="text-xs font-bold"> KEEP*IT*FLY DROP 1 LOOKBOOK </p><a
                              href="{{route('lookbook')}}" class="text-xs">VIEW NOW</a>
                          </div>
                        </div>

                      </div>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </li>
        <li class="relative group py-2"><button
            class="curseur focus:text-black focus:font-bold extra antialiased text-xs pl-2 py-2 rounded-md hover:font-bold"
            aria-haspopup="true"> BRAND </button>
          <div
            class="w-screen absolute -left-[233px] top-2 transition group-hover:translate-y-[13px] translate-y-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-700 ease-in-out group-hover:transform z-50 min-w-[560px] transform">
            <div class="invisible pt-8">fafefefefaefaefaefaeffefaefef</div>
            <div class="relative top-6 pl-9 py-3 bg-white border-b border-t border-tiret2 w-screen">
              <div class="relative z-10">
                <div class="flex justify-start gap-56">
                  <div class="grid grid-cols-3 gap-[88px]">
                    <ul class="flex gap-9">

                      <li><a href="{{route('about')}}"
                          class="text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                          ABOUT </a></li>
                    <li><a href="{{route('media')}}"
                            class="text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                            MEDIA </a></li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </li>
      </ul>
    </nav>
    <div>
      <nav><a aria-current="page" href="{{route('home')}}"
          class="router-link-active router-link-exact-active mx-auto"><img src="{{asset('files/logo.e4b8d2f2.svg')}}"
            height="38"></a></nav>
    </div>
    <nav>
      <ul class="flex items-center justify-center space-x-5">
        <li class="relative group py-2"><button
            class="curseur focus:text-black focus:font-bold extra antialiased text-xs pl-6 py-2 rounded-md hover:font-bold"
            aria-haspopup="true"> ACCOUNT </button>

          <div
            class="absolute -right-[195px] top-2 transition group-hover:translate-y-[11px] translate-y-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-700 ease-in-out group-hover:transform z-50 min-w-[560px] transform">
            <div class="invisible pt-8">fafefefefaefaefaefaeffefaefef</div>
            <div class="relative top-6 pl-9 py-3 bg-white border-b border-t border-tiret2 w-screen">
              <div class="relative z-10">
                <div class="flex justify-end gap-56">
                  <div class="grid grid-cols-1">
                    <ul class="flex gap-9">
                        @if (Auth::check())

                        <li>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <a href="route('logout')" onclick="event.preventDefault();
                                this.closest('form').submit();"
                                class="text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600"
                                 > LOGOUT</a>

                            </form>
                        </li>

                        @else

                        <li><a href="{{ route('login') }}"
                            class="text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                            LOGIN</a></li>
                        <li>
                            <a href="{{ route('register') }}"
                            class="text-xs text-block pr-9 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600">
                           REGISTER</a></li>



                    @endif
                    </ul>

                  </div>
                </div>
              </div>
            </div>
          </div>
        </li><button href="#" class="relative group py-2 mx-auto">
            <img
            class="focus:text-black focus:font-bold extra antialiased text-xs py-2 rounded-md hover:font-bold"
            aria-haspopup="true"
            src=" @if (Auth::check())  {{ asset('files/' .  getUserImageCurrency() . '') }} @else {{ asset('files/' .  getImageCurrency() . '') }} @endif  " height="18">

          <div
            class="absolute -right-[153px] top-2 transition group-hover:translate-y-[16px] translate-y-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-700 ease-in-out group-hover:transform z-50 min-w-[560px] transform">
            <div class="invisible pt-8">fafefefefaefaefaefaeffefaefef</div>
            <div class="relative top-6 pl-9 py-2 bg-white border-b border-t border-tiret2 w-screen">
              <div class="relative z-10">
                <div class="flex justify-end gap-56">
                  <div class="grid grid-cols-1">
                    <ul class="flex gap-9">

                        <li><a href="{{ route('change_currency', ['currency' => '€', 'id'=> 1]) }}"
                          class="flex justify-center items-center text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600"><img
                            class="mr-[10px]" src="{{asset('files/flag-eur.eb1348a4.svg')}}" alt=""><a
                            href="{{ route('change_currency', ['currency' => '€', 'id'=> 1]) }}" class="text-xs">EUROPEAN</a></a></li>

                            <li><a href="{{ route('change_currency', ['currency' => 'CHF', 'id'=> 2]) }}"
                          class="flex justify-center items-center text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600"><img
                            class="mr-[10px]" src="{{asset('files/flag-swz.dc144f59.svg')}}" alt=""><a
                            href="{{ route('change_currency', ['currency' => 'CHF', 'id'=> 2]) }}" class="text-xs">SWITZERLAND</a></a></li>

                            <li><a href="{{ route('change_currency', ['currency' => '£', 'id'=> 3]) }}"
                          class="flex justify-center items-center text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600"><img
                            class="mr-[10px]" src="{{asset('files/flag-uk.3248720d.svg')}}" alt=""><a
                            href="{{ route('change_currency', ['currency' => '£', 'id'=> 3]) }}" class="text-xs">UNITED KINGDOM</a></a></li>

                            <li><a href="{{ route('change_currency', ['currency' => '$', 'id'=> 4]) }}"
                          class="flex justify-center pr-9 items-center text-xs text-block p-2 -mx-2 rounded-lg hover:bg-gradient-to-br hover:from-indigo-50 hover:to-pink-50 hover:via-blue-50 transition ease-in-out duration-300 text-gray-800&gt; hover:text-indigo-600"><img
                            class="mr-[10px]" src="{{asset('files/flag-usa.1c85eb67.svg')}}" alt=""><a
                            href="{{ route('change_currency', ['currency' => '$', 'id'=> 4]) }}" class="text-xs">USA / REST OF THE
                            WORLD</a></a></li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </button><button href="#" class="relative group py-2 mx-auto"><img
            class="focus:text-black focus:font-bold extra antialiased text-xs py-2 rounded-md hover:font-bold"
            aria-haspopup="true" src="{{asset('files/search.c68780b6.svg')}}" height="18">
          <div
            class="absolute -right-[116px] top-2 transition group-hover:translate-y-[16px] translate-y-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-700 ease-in-out group-hover:transform z-50 min-w-[560px] transform">
            <div class="invisible pt-8">fafefefefaefaefaefaeffefaefef</div>
            <div class="relative mt-1 top-6 py-3 border-b border-t border-tiret2 h-screen w-screen">
              <div
                class="relative mt-1 top-6 py-3 bg-black border-b opacity-50 border-t border-tiret2 h-screen w-screen">
              </div>
              <div class="relative z-10">
                <div class="flex justify-start">
                  <div class="relative">
                    <div class="-mt-7 z-20 fixed w-screen pt-8 top-20">
                      <div class="">
                        <form>
                          <div class="relative"><input type="text"
                              class="block pl-9 pr-9 focus:outline-0 focus:border-gray py-2.5 w-full text-base text-black bg-white border-b border-t border-tiret2"
                              placeholder="Search for...">
                            <div class="absolute right-0 top-3 pr-9 pb-10"><img
                                src="{{asset('files/search.c68780b6.svg')}}" height="18"></div>
                          </div>
                        </form>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </button>

        @if (Auth::check())
        <a href="{{ route('account')}}" class="relative group py-2 mx-auto">
            <img class="focus:text-black focus:font-bold extra antialiased text-xs py-2 rounded-md hover:font-bold"
            aria-haspopup="true" src="{{asset('files/profil.c74a5317.svg')}}" height="18"></a>
        @endif

        <a href="{{ route('wishlists')}}" class="relative group py-2 mx-auto">
            <img class="focus:text-black focus:font-bold extra antialiased text-xs py-2 rounded-md hover:font-bold"
            aria-haspopup="true" src="{{asset('files/wish.639c7b91.svg')}}" height="18"></a>

        <a href="{{route('add_cart')}}" class="relative group py-2 mx-auto"><img
            class="focus:text-black focus:font-bold extra antialiased text-xs py-2 rounded-md hover:font-bold"
            aria-haspopup="true" src="{{asset('files/cart.95d40601.svg')}}" height="18">
            <span class="cart_qty_cls">
                {{getNumberProductCart()}}
            </span>
        </a>

      </ul>
    </nav>
  </header>
